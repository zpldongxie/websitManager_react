...
const SelectImage = ({ title, visible, contentHtml, defaultImg, onOk, onCancel }: PropsType) => {
  ...
  useEffect(() => {
    if (!selectedUrl && defaultImg) setSelectedUrl(defaultImg);
  }, [defaultImg, visible]);
  ...
  return (
    <Modal
      ...
      visible={visible}
      onOk={() => {
        onOk(selectedUrl === 'none' ? '' : selectedUrl);
        onCancel();
      }}
      onCancel={onCancel}
    >
      <Row className={styles.listCon} gutter={10}>
        {list.map((item: string) => (
          <Col
            key={item}
            onClick={() => {
              setSelectedUrl(item === selectedUrl ? '' : item);
            }}
          >
            <div
        ...
        <Input
          ...
          addonAfter={
            <Button
              ...
              onClick={() => {
                if (netInputValue) setList([...list, netInputValue]);
              }}
            >
              添加
            </Button>
